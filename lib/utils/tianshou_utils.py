...
class decision_transformer():
    def __init__(self,tianshou_obs):
        ...
        print(os.getcwd())
        self.model_dt.load_state_dict(torch.load("/home/wolz/projects/NeuroLS_DecisionTransformer/trained_model_nls.pt", map_location=torch.device(self.device))) 
    ...
    def update_states(self, state):
        ...
        if (self.all_states is None):
            self.all_states = self.state.to(self.device)
        else:
